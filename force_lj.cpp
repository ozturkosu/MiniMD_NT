...
void ForceLJ::compute(Atom &atom, Neighbor &neighbor, Comm &comm, int me)
{
 ...
 // visited = (int *) malloc(nall*sizeof(int));

  if(evflag) {

    if (hma == 5)
      return compute_NT5<1>(atom, neighbor, me);
    if (hma == 4)
      return compute_NT4<1>(atom, neighbor, me);
    if (hma == 3)
      return compute_NT3<1>(atom, neighbor, me);
    if (hma == 2)
      return compute_NT2<1>(atom, neighbor, me);
    if (hma == 1)
      return compute_NT1<1>(atom, neighbor, me);
...
//Ashish Jha, ashish.jha@intel.com, Intel Corporation
    ...
    if (hma == 5)
      return compute_NT5<0>(atom, neighbor, me);
    if (hma == 4)
      return compute_NT4<0>(atom, neighbor, me);
    if (hma == 3)
      return compute_NT3<0>(atom, neighbor, me);
    if (hma == 2)
      return compute_NT2<0>(atom, neighbor, me);
    if (hma == 1)
      return compute_NT1<0>(atom, neighbor, me);
...
//  NT version of compute
//  - MPI + OpenMP
//  - based on the on-node NT algorithm - Tower-Plate form
//  - uses coloring to prevent race conditions
//  - pro: no temp vectors
//  - con: very bad data locality
//  - WILL use temporary variable for summing up fi
//  - WILL enable vectorization by:
//     -getting rid of 2d pointers
//     -use pragma simd to force vectorization of inner loop
template<int EVFLAG>
void ForceLJ::compute_NT1(Atom &atom, Neighbor &neighbor, int me)
{
  //int* neighs;
...
//printf("tid : %5d\n", tid);
  ...
  const int nall = atom.nlocal + atom.nghost;
  const int max_colors = neighbor.max_colors;
/*   FILE *file;
  if ((threads->mpi_me ==0)&&(neighbor.ncalls==1))
     file = fopen("pairsNT1.txt", "w");  */
...
#pragma omp for schedule(guided)
    for (int w = 0; w < neighbor.num_cbins[c]; w++) {
      const int binno = neighbor.cbin_list[c][w];
      int * RESTRICT neighs = &neighbor.neighbors[binno*neighbor.maxneighs*2];
      const int numneighs = neighbor.numneigh[binno];

#ifdef USE_SIMD
      //     #pragma simd
#endif      
      for (int k = 0; k < numneighs; k+=2) { //pairs++;
	const int i = neighs[k];
	const int j = neighs[k+1];
    ...
    // there is an implicit barrier at the end of each omp for
  }
...
//     -use pragma simd to force vectorization of inner loop
template<int EVFLAG>
void ForceLJ::compute_NT2(Atom &atom, Neighbor &neighbor, int me)
{
  ...
  /*FILE * file;  
  if ((threads->mpi_me ==0)&&(neighbor.ncalls==1)) 
     file = fopen("pairsNT2.txt", "w");*/
 // printf("tid : %d \n", tid);
  const int nlocal = atom.nlocal;
  const int nall = atom.nlocal + atom.nghost;
  const int numcbin = neighbor.numcbin;
  MMD_float* x = &atom.x[0][0];
  MMD_float* f = &atom.f[0][0];
  ...
  MMD_float t_energy = 0;
  MMD_float t_virial = 0;
 // printf("numcbin: %d, nlocal : %d, nall : %d,\n",numcbin, nlocal, nall );
  //Neighbors on the NT structure
	 /* grab the central bin */
#pragma omp for schedule(guided)
  for(int c = 0; c < numcbin; c++){
        const int cbino = neighbor.cbin[c];  // central bin no
        int* RESTRICT neighs = &neighbor.neighbors[cbino * neighbor.maxneighs * 2];  // neighbors of an atom in the central bin
        const int numneighs = neighbor.numneigh[cbino];
        MMD_float fix = 0.0;
        MMD_float fiy = 0.0;
        MMD_float fiz = 0.0;

//#ifdef USE_SIMD
//#pragma simd reduction (+: fix,fiy,fiz,t_energy,t_virial)
//#endif
       for(int k = 0; k < numneighs; k+=2) { //pairs++;
          const int i = neighs[k];
          const int j = neighs[k+1];
...
//	    fprintf(file, " i: %5d, j: %5d\n", i, j); 
            ...
            fiz = delz * force;
	    #pragma omp atomic	
            f[i * PAD + 0] += fix ;//delx * force;
	    #pragma omp atomic
            f[i * PAD + 1] += fiy; //dely * force;
	    #pragma omp atomic
            f[i * PAD + 2] += fiz; //delz * force;
	    #pragma omp atomic
            f[j * PAD + 0] -= fix; //delx * force;
	    #pragma omp atomic
            f[j * PAD + 1] -= fiy; //dely * force;
	    #pragma omp atomic
            f[j * PAD + 2] -= fiz; //delz * force;
...
//duplicate force vector
template<int EVFLAG>
void ForceLJ::compute_NT3(Atom &atom, Neighbor &neighbor, int me)
{

  int xi, yi, zi;
  int tid = omp_get_thread_num();
  int num_omp_threads = threads->omp_num_threads;
 // printf("tid : %d numth: %d, %d \n", tid, num_omp_threads, PAD);
  const int nall = atom.nlocal + atom.nghost;
  const int numcbin = neighbor.numcbin;
  const int atoms_per_thread = neighbor.atoms_per_thread;  // equal length for each thread : duplicate array
 /* printf("numcbin_per_thread: %5d" ,numcbin_per_thread);
  FILE *file;
  if  ((threads->mpi_me ==0)&&(neighbor.ncalls==1)) file = fopen("pairsNT3.txt", "w");
  int m =0;*/
  MMD_float* x = &atom.x[0][0];
  MMD_float* f = &atom.f[0][0];
  MMD_float* tempf = &neighbor.tempF[tid * atoms_per_thread*PAD];  // 
  //MMD_float* tempf = &atom.f[tid * atoms_per_thread][0];  // 
...
//  #pragma omp for schedule(static, 100)
  ...
  MMD_float t_energy = 0;
  MMD_float t_virial = 0;
  //  printf("numcbin: %d, nlocal : %d, nall : %d, atoms_per_thread : %d \n",numcbin, nlocal, nall, atoms_per_thread );
  //Neighbors on the NT structure
	 /* grab the central bin */
  #pragma omp for schedule(guided )
  for(int c = 0; c < numcbin; c++){
...
//	 m++;// tempatom[++m] = i;
  ...
  //     	  fprintf(file, " i: %5d, j: %5d\n", i, j);
            ...
            fiy = dely * force;
            fiz = delz * force;
            tempf[i * PAD + 0] += fix ;//delx * force;
            tempf[i * PAD + 1] += fiy; //dely * force;
            tempf[i * PAD + 2] += fiz; //delz * force;

            tempf[j * PAD + 0] -= fix; //delx * force;
            tempf[j * PAD + 1] -= fiy; //dely * force;
            tempf[j * PAD + 2] -= fiz; //delz * force;
	
            if(EVFLAG) {
              const MMD_float scale = 1.0;//( (i < nlocal) && (j < nlocal) ) ? 1.0 : 0.5;
              t_energy += scale * (4.0 * sr6 * (sr6 - 1.0)) * epsilon;
              t_virial += scale * (delx * delx + dely * dely + delz * delz) * force;
           }
        }
...
#pragma omp for schedule(guided)
  for(int ai = 0; ai < nall; ai++)
  for(int ti =0; ti < num_omp_threads; ti++){
	int const offset = ti * atoms_per_thread * PAD;
//	   printf(" ti: %5d, tid: %5d\n" , ti,tid);// l: %5d, i: %5d\n", ai, l, i);
            f[ai * PAD + 0] += neighbor.tempF[offset + ai *  PAD +0] ; 
            f[ai * PAD + 1] += neighbor.tempF[offset + ai *  PAD +1] ; 
            f[ai * PAD + 2] += neighbor.tempF[offset + ai *  PAD +2] ; 
     }
/*if ((threads->mpi_me ==0)&&(neighbor.ncalls==1))
  for(int ai = 0; ai < nall; ai++){
//	   printf(" ai: %5d, %5d\n" , ai,size);// l: %5d, i: %5d\n", ai, l, i);
           fprintf(file, "%.3f, %.3f, %.3f\n",  f[ai * PAD + 0],  f[ai * PAD + 1],  f[ai * PAD + 2]); 
} */
...
template<int EVFLAG>
void ForceLJ::compute_NT4(Atom &atom, Neighbor &neighbor, int me)
{
  //int* neighs;
  int tid = omp_get_thread_num();
//printf("tid : %5d\n", tid);
...
//#pragma omp for schedule(guided)
   ...
   for(int i = 0; i < nall; i++) {
        int* RESTRICT neighs = &neighbor.neighbors[i * neighbor.maxneighs];
...
template<int EVFLAG>
void ForceLJ::compute_NT5(Atom &atom, Neighbor &neighbor, int me)
{
  ...
  const int atoms_per_thread = neighbor.atoms_per_thread;  // equal length for each thread : duplicate array
FILE *file;
if  (threads->mpi_me ==0) file = fopen("pairs.txt", "w");
int m =0;
  MMD_float* x = &atom.x[0][0];
  MMD_float* f = &atom.f[0][0];
  MMD_float* tempf = &neighbor.tempF[tid * atoms_per_thread*PAD];  // 
