...
int read_lammps_data(Atom &atom, Comm &comm, Neighbor &neighbor, 
		     Integrate &integrate, Thermo &thermo, char* file, 
		     int units);
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if ((strcmp(argv[i], "-hma") == 0) || (strcmp(argv[i], "--hma") == 0)) {
      hma = atoi(argv[++i]);
      continue;
    }
  ...
  if (me == 0) {
    ...
    printf("\t# nbinx nbiny nbinz = %d %d %d\n", 
  ...
  /* info on which force computation routine will be used */
  if (me == 0) {
  printf("using lj force routine: "); 
  if (force->evflag) {
    
    if (force->use_oldcompute)
      printf("compute_original<1> - MPI only, not vectorizable\n");
    
    if ((hma == 1)||(hma == 2)) 
      printf("compute_hma<1> - "
		 "MPI+OpenMP, based on the on-node NT algorithm - "
	         "Tower-Plate form  - coloring to prevent race conditions\n");
    else if (neighbor.halfneigh) {
      if (neighbor.ghost_newton) {
        if (threads.omp_num_threads > 1)
          printf("compute_halfneigh_threaded<1,1> - "
		 "MPI+OpenMP (atomics for fj update), "
		 "temp var to update fi, enables vectorization\n");
        else
          printf("compute_halfneigh<1,1> - "
		 "MPI only, temp var to update fi, enables vectorization\n");
      } 
      else {
        if (threads.omp_num_threads > 1)
	  printf("compute_halfneigh_threaded<1,0> - "
		 "MPI+OpenMP (atomics for fj update), "
		 "temp var to update fi, enables vectorization\n");
        else
          printf("compute_halfneigh<1,0> - "
		 "MPI only, temp var to update fi, enables vectorization\n");
      }
#ifdef KNC_FORCE_INTRINSIC_SWGS
    } 
    else {
      printf("compute_fullneigh_intrinsic_SWGS<1> - "
	     "optimised  with INTRINSICs, MPI + OpenMP (using full nbrlists) ",
    ...
    if (force->use_oldcompute)
      printf("compute_original<0> - MPI only, not vectorizable\n");

    if ((hma == 1)||(hma == 2)) 
      printf("compute_hma<0> - "
	         "MPI+OpenMP, based on the on-node NT algorithm - "
	         "Tower-Plate form  - coloring to prevent race conditions\n");
    else if (neighbor.halfneigh) {
      if (neighbor.ghost_newton) {
        if (threads.omp_num_threads > 1)
          printf("compute_halfneigh_threaded<0,1> - "
		 "MPI+OpenMP (atomics for fj update), "
		 "temp var to update fi, enables vectorization\n");
        else
          printf("compute_halfneigh<0,1> - "
		 "MPI only, temp var to update fi, enables vectorization\n");
      } 
      else {
        if (threads.omp_num_threads > 1)
	  printf("compute_halfneigh_threaded<0,0> - "
		 "MPI+OpenMP (atomics for fj update), "
		 "temp var to update fi, enables vectorization\n");
        else
          printf("compute_halfneigh<0,0> - "
		 "MPI only, temp var to update fi, enables vectorization\n");
      }
#ifdef KNC_FORCE_INTRINSIC_SWGS
    } 
    else {
      printf("compute_fullneigh_intrinsic_SWGS<0> - "
	     "optimised  with INTRINSICs, MPI + OpenMP (using full nbrlists) ",
	     "OA-gets rid of fj update (read/write to memory) ", 
	     "temp var to update fi, enables vectorization\n");
    }
#else
    } 
    else {
      printf("compute_fullneigh<0> - " 
	     "MPI + OpenMP (using full neighborlists), "
	     "gets rid of fj update (read/write to memory), "
	     "temp var to update fi, enables vectorization\n");
    }
  ...
  if (me == 0) {
    double time_other = timer.array[TIME_TOTAL] - timer.array[TIME_FORCE] - 
      timer.array[TIME_NEIGH] - timer.array[TIME_COMM];
    
    printf("\n\n");
    printf("# Performance Summary:\n");
    printf("# MPI_proc OMP_threads nsteps natoms" 
	   "t_total t_force t_neigh t_comm t_other"
	   "performance perf/thread grep_string t_extra\n");
    printf("%i %i %i %i %lf %lf %lf %lf %lf %lf %lf PERF_SUMMARY %lf\n\n\n",
	   ...
	   timer.array[TIME_NEIGH], timer.array[TIME_COMM], time_other,
           1.0 * natoms * integrate.ntimes / timer.array[TIME_TOTAL], 
	   1.0 * natoms * integrate.ntimes / 
	   (timer.array[TIME_TOTAL] * nprocs * num_threads), 
	   timer.array[TIME_TEST]);
  }

  if (yaml_output)
    output(in, atom, force, neighbor, comm, thermo, 
	   integrate, timer, screen_yaml);
